/**
 * Prayer times. Real from PHASE 2 — `adhan` computes adhan locally from the
 * mosque's coordinates and `prayerConfig`; iqamah comes from the mosque's own
 * config on top of that.
 *
 * Two halves:
 *   - `buildPrayerTable` produces the `PrayerTable` shape the API returns. The
 *     mock client calls it; the server will run the same maths in PHASE 4.
 *   - `nextPrayerFrom` reads a `PrayerTable` back into instants for the
 *     countdown. It only knows the API shape, so it survives PHASE 5 untouched.
 *
