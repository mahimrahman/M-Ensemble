...
All collections use string `_id`. `timestamps: true` is fine everywhere but the
contract only exposes `createdAt` where listed.
...
| iqamahconfigs  | 10   | 5 Khadija · 5 Madina, one of which is Madina Isha `effectiveFrom: 2026-09-08` |
...
- [ ] `npm run seed` prints the counts in the table above
- [ ] every route in this document answers with the right shape from Postman /
      a `.http` file checked into `apps/server/requests.http`
- [ ] the vitest suite above is green, including the race
- [ ] `npm run typecheck` and `npm test` pass at the repo root
