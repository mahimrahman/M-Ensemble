...
| -------------- | ---- | --------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| mosques        | 10   | Khadija, Madina, Salahouddine, CIIC, Verdun, Fatima, Al-Rawdah, Laval, Ottawa, Toronto                                                                          |
| users          | 20   | all sign in with `mensemble`                                                                                                                                    |
| follows        | 23   | user_001 follows five: Khadija, Madina, Salahouddine, CIIC, Verdun                                                                                              |
| memberships    | 7    | six coordinators (one per Montreal mosque) plus user_001 member @ Khadija                                                                                       |
| posts          | 35   | 30 upcoming, 5 past                                                                                                                                             |
...
- **race:** seed the demo shift (`post_002`, `slotsNeeded: 4`) at 3/4, fire 10 concurrent `POST /posts/post_002/signup` as 10 different users → exactly one 201, nine 409 `FULL`, `slotsFilled === 4`. The test finds the post by shape (`type: 'volunteer', slotsNeeded: 4`) rather than by id, so renumbering the fixtures cannot silently aim it at a post with no limit — which would let all ten through and still look green.
...
- dashboard / members / outcomes on the seeded Khadija match what the app shows on mocks (the mock client in `apps/mobile/src/api/mock/mockClient.ts` is the oracle — compare its derived views for Khadija against the API, signed in as khadija.mosque@gmail.com)
...
The fan-out itself is verified as far as it can be without a device: creating a
post selects the right recipients, chunks them, reaches Expo's API and logs the
ticket. Registering a deliberately invalid token and creating a post produced
`[push] sent 1, 1 rejected` with Expo naming the bad token — every step but the
handset. Only the last box needs a real `ExponentPushToken`.

The five derived views were also diffed against the mock oracle rather than read
by eye: dashboard, members, outcomes, roster and the prayer table each serialise
identically from the server and from `mockClient` on the same fixtures.
