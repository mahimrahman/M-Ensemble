...
1. **Feed** — prayer card counts down to Khadija's next iqamah. Filter to
   *Volunteer*. "Iftar setup" reads 1 of 4.
...
5. **Kitchen cleanup crew** is full — the button reads *All slots filled*.
...
same way the server will, and the screen shows *Just filled up* then reloads.
...
that verifies the *iqamah config*, which no library can.
